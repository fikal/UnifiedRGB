...
public sealed class SupportService
{
    ...
    public async Task<(bool Ok, string Message)> SendBundleAsync(string? note, Action<string> status)
    {
        ...
        // No backend (public/open-source build): save the bundle locally so
        // the user can attach it to a GitHub issue. Nothing leaves the machine.
        status("saving bundle...");
        try
        {
            string outPath = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.Desktop),
                $"UnifiedRGB-diagnostic-{DateTime.Now:yyyyMMdd-HHmmss}.txt");
            string header = string.IsNullOrWhiteSpace(note) ? "" : $"note: {note}\r\n\r\n";
            await File.WriteAllTextAsync(outPath, header + bundle);
            return (true, $"saved to {outPath} — attach it to a GitHub issue");
        }
        catch (Exception ex)
        {
